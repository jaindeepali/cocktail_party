...
if only > 1
  ...
  if jumpWhitening == 3,
    ...
    outputDir = strcat('../sound_files/ica_output/', 'singleSine/');
    f = figure('visible','off');
    a = whitesig(2, :);
    hist(a);
    title('Histogram for whitened signal 2');
    xlabel('Whitened signal value')
    ylabel('Count in histogram')
    filename=sprintf(strcat(outputDir,'hist2.png'),f);
    print(filename);
  end
